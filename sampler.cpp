...
Sampler::map_samples_to_sphere(void) {
	...
	for (int j = 0; j < num_samples * num_sets; j++) {
    	...
    	x 	= r * cos(phi);
    	y 	= r * sin(phi);
		sphere_samples.push_back(vec3(x, y, z)); 
	}
}
...
vec2 Sampler::sample_unit_square(void) {
...
vec2 Sampler::sample_unit_disk(void) {
	if (count % num_samples == 0)  									// start of a new pixel
...
vec3 Sampler::sample_hemisphere(void) {
	if (count % num_samples == 0)  									// start of a new pixel
		jump = (rand_int() % num_sets) * num_samples;
...
vec3 Sampler::sample_sphere(void) {
	if (count % num_samples == 0)  									// start of a new pixel
		jump = (rand_int() % num_sets) * num_samples;
